...
機体側は設定された周期でテレメトリを送信し、TOP立上りによる即時送信は行いません。
UARTはRX/TXへ分割し、RXは送信処理やAUX待機と独立したタスクで常時読み出します。
TXは書き込み直前にAUX Highを必須確認し、補助対策としてchecksumが正常なフレームの
最終受信から60 ms待機した後にもAUXを再確認します。未知コマンドも通信活動として
待機対象にします。不正フレームおよびUART受信エラー発生時は組み立て途中のフレームを
破棄し、次のHeaderから再同期します。延期中に失効した周期送信は連続実行しません。
...
反映せず受信数だけ記録します。0x200 bit 0のTOP状態はPayloadへ反映しますが、立上り時の
即時送信には使用しません。
