...
mod tests {
    ...
    #[test]
    fn parses_gst_with_empty_fields() {
        let data = parse_gst(b"$GPGST,082356.00,1.8,,,,1.7,1.3,2.2*7E").unwrap();

        assert_eq!(data.range_rms, Some(1.8));
        assert_eq!(data.std_major, None);
        assert_eq!(data.std_minor, None);
        assert_eq!(data.orient, None);
        assert_eq!(data.std_lat, Some(1.7));
        assert_eq!(data.std_long, Some(1.3));
        assert_eq!(data.std_alt, Some(2.2));
    }

    #[test]
    fn parses_gst_with_gn_talker() {
        let data = parse_gst(b"$GNGST,082356.00,1.8,,,,1.7,1.3,2.2*60").unwrap();
    ...
    #[test]
    fn rejects_gst_checksum_mismatch() {
        let err = parse_gst(b"$GPGST,082356.00,1.8,,,,1.7,1.3,2.2*00").unwrap_err();

        assert_eq!(err, GgaParseError::InvalidChecksum);
    }

    #[test]
    fn rejects_non_gst_sentence() {
        let err = parse_gst(b"$GPGGA,082356.00,,,,,,0,,,,M,,M,,*6E").unwrap_err();

        assert_eq!(err, GgaParseError::InvalidTalker);
    }
}
